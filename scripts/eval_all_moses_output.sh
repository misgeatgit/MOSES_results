...
for fd in $(seq 1 $Kfd); do
    for cfile in $exp_dir/anal/fd_${fd}to${Kfd}*.combo; do
        for smp in $samples; do
            ...
            CMD=" /home/addis-ai/opencog_ocpkg/opencog/build/opencog/comboreduct/main/eval-table -i $ifile -C $cfile -o $ofile  -u f280 " #my modification(Misgana) changed value of -u from out to f280 ,delete --labels 1 option
